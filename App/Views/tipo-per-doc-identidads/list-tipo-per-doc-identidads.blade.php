...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-tipo-per-doc-identidads') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('tipo-per-doc-identidad-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de tipo-per-doc-identidads
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            ...
            <?php
              ...
              if(!empty($row->tpdi_publicar)){
                if($row->tpdi_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->tpdi_descripcion }} </td> 
                <td> {{ $row->tpdi_glosa }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->tpdi_estado]["class"] ?>"> 
                    <?php echo $status[$row->tpdi_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('tipo-per-doc-identidad-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->tpdi_descripcion}}`,`<?php echo $title_estado ?>`,`{{$row->tpdi_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('tipo-per-doc-identidad-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> tipo per doc identidad: <strong> ${textRow} </strong> ?`;
