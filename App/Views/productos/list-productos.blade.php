...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      ...
      <a href="{{ route('producto-new') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td nowrap>
                  <a class="btn btn-outline-primary btn-sm lh-1 btn-table <?php echo $class_disabled; ?>"
                    href="{{ route('producto-edit',['id' => $row->id]) }}" title="Editar">
                    <i class="fas fa-pencil-alt"></i>
                  </a>
                  <button class="btn btn-outline-danger btn-sm lh-1 btn-table"
                  onclick="modalDelete({{$row->id}}, `{{$row->sub_categoria_id}}`,`<?php echo $title_estado ?>`,`{{$row->estado}}`);" title="<?php echo $title_estado; ?>">
...
<script>
  ...
  function modalDelete(id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> de productos: <strong> ${textRow} </strong> ?`;
