...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-per-telefonos') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('per-telefono-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de per-telefonos
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            ...
            <?php
              ...
              if(!empty($row->pt_publicar)){
                if($row->pt_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                <td> {{ str_pad($row->persona_id, 3, "0", STR_PAD_LEFT) }} </td> 
                <td> {{ $row->tipo_telefono_id }} </td> 
                <td> {{ $row->pt_jerarquia }} </td> 
                <td> {{ $row->pt_telefono }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->pt_estado]["class"] ?>"> 
                    <?php echo $status[$row->pt_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('per-telefono-edit',['id' => $row->persona_id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->persona_id}}, `{{$row->tipo_telefono_id}}`,`<?php echo $title_estado ?>`,`{{$row->pt_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('per-telefono-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> per telefono: <strong> ${textRow} </strong> ?`;
