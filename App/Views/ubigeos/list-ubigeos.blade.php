...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-ubigeos') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('ubigeo-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de ubigeos
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            <thead>
              <tr>
                <th width="60"> Id </th> 
                <th> Pais Id </th> 
                <th> Ubigeo Id Padre </th> 
                <th> Codigo </th> 
                <th> Ubigeo </th> 
                <th> Descripcion </th> 
                <th> Tipo Ubigeo Id </th> 
            ...
            <?php
              ...
              if(!empty($row->ubi_publicar)){
                if($row->ubi_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->pais_id }} </td> 
                <td> {{ $row->ubigeo_id_padre }} </td> 
                <td> {{ $row->ubi_codigo }} </td> 
                <td> {{ $row->ubi_ubigeo }} </td> 
                <td> {{ $row->ubi_descripcion }} </td> 
                <td> {{ $row->tipo_ubigeo_id }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->ubi_estado]["class"] ?>"> 
                    <?php echo $status[$row->ubi_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('ubigeo-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->pais_id}}`,`<?php echo $title_estado ?>`,`{{$row->ubi_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('ubigeo-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> ubigeo: <strong> ${textRow} </strong> ?`;
