...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-per-doc-identidads') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('per-doc-identidad-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de per-doc-identidads
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            <thead>
              <tr>
                <th width="60"> Id </th> 
                <th> Persona Id </th> 
                <th> Tipo Per Doc Identidad Id </th> 
                <th> Jerarquia </th> 
                <th> Numero </th> 
                <th> Fecha Emision </th> 
            ...
            <?php
              ...
              if(!empty($row->pdi_publicar)){
                if($row->pdi_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->tipo_per_doc_identidad_id }} </td> 
                <td> {{ $row->pdi_jerarquia }} </td> 
                <td> {{ $row->pdi_numero }} </td> 
                <td> {{ $row->pdi_fecha_emision }} </td> 
                <td> {{ $row->pdi_fecha_caducidad }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->pdi_estado]["class"] ?>"> 
                    <?php echo $status[$row->pdi_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('per-doc-identidad-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->persona_id}}`,`<?php echo $title_estado ?>`,`{{$row->pdi_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('per-doc-identidad-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> per doc identidad: <strong> ${textRow} </strong> ?`;
