...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-bitacoras') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('bitacora-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de bitacoras
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            ...
            <?php
              ...
              if(!empty($row->bit_publicar)){
                if($row->bit_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->table_id }} </td> 
                <td> {{ $row->table }} </td> 
                <td> {{ $row->computer_ip }} </td> 
                <td> {{ $row->new_value }} </td> 
                <td> {{ $row->old_value }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->bit_estado]["class"] ?>"> 
                    <?php echo $status[$row->bit_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('bitacora-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->user_id}}`,`<?php echo $title_estado ?>`,`{{$row->bit_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('bitacora-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> bitacora: <strong> ${textRow} </strong> ?`;
