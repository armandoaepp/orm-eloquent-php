...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-convenios') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('convenio-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header bg-white">
          <i class="fa fa-align-justify"></i> Lista de convenios
        </div>
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            ...
            <?php
              ...
              if(!empty($row->con_publicar)){
                if($row->con_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->tipo_convenio_id }} </td> 
                <td> {{ $row->con_nombre }} </td> 
                <td> {{ $row->con_caracteristica }} </td> 
                <td> {{ $row->con_precio }} </td> 
                <td class="text-center">
                  <?php echo $publicado; ?>
                </td>
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->con_estado]["class"] ?>"> 
                    <?php echo $status[$row->con_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('convenio-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->tipo_convenio_id}}`,`<?php echo $title_estado ?>`,`{{$row->con_estado}}`);" title="<?php echo $title_estado; ?>" >
                        <i class="far fa-trash-alt"></i> <?php echo $title_estado; ?>
                      </a>
                      <a class="dropdown-item <?php echo $class_disabled; ?>" href="#" onclick="modalPublicar(event, <?php echo $row->id ?>, `<?php echo $row->tipo_convenio_id ?>` ,`<?php echo $title ?>`, `<?php echo $row->con_publicar;  ?>`);" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('convenio-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> convenio: <strong> ${textRow} </strong> ?`;
...
<script>
  ...
  (function ($) 
  {
    ...
    function processFormModalPublicar(event) {
      ...
      let url_api_pub = "{{ route('convenio-publish') }}";
