...
<div class="container-fluid">
  <div class="row">
    <div class="col-12 mb-3">
      <a href="{{ route('admin-personas') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        <i class="fas fa-list-ul"></i>
        Listar
      </a>
      <a href="{{ route('persona-create') }}" class="btn btn-outline-secondary btn-sm btn-bar" role="button">
        ...
        <div class="card-body">          
          ...
          <table id="dataTableList" class="table table-sm table-hover table-bordered dt-responsive nowrap" style="width: 100%;">
            ...
            <?php
              ...
              if(!empty($row->per_publicar)){
                if($row->per_publicar == "S"){
            ...
            ?>
              ...
              <tr class="<?php echo $class_estado; ?>">
                ...
                <td> {{ $row->per_fecha_nac }} </td> 
                <td> {{ $row->per_tipo }} </td> 
                <td class="text-center">
                  <span class="badge badge-pill <?php echo $status[$row->per_estado]["class"] ?>"> 
                    <?php echo $status[$row->per_estado]["title"] ?> 
                  </span>
                </td>
                <td class="text-center">
                  <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item <?php echo $class_disabled; ?>"href="{{ route('persona-edit',['id' => $row->id]) }}" >
                        <i class="far fa-edit"></i> Editar
                      </a>
                      <a class="dropdown-item " href="#" onclick="modalDelete(event,{{$row->id}}, `{{$row->per_id_padre}}`,`<?php echo $title_estado ?>`,`{{$row->per_estado}}`);" title="<?php echo $title_estado; ?>" >
...
<script>
  ...
  (function ($) {
    function processFormModal(event) {
      ...
      let url_api = "{{ route('persona-delete') }}";
  ...
  function modalDelete(event, id, textRow, title, estado) {
    ...
    var text = `¿Esta seguro de <strong> ${title} </strong> persona: <strong> ${textRow} </strong> ?`;
